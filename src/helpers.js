...
export const encryptObject = async (data_obj, pub_key) => {
  var content_encoder = new TextEncoder()
    var new_format = JSON.stringify(data_obj)
    var data_buf = content_encoder.encode(new_format)
    var key_buf = await generate_random_bytes(256)

    // Encrypt data segments
    var encrypted_data_obj =
    ...
    return arweave.utils.concatBuffers([encrypted_key, encrypted_data_obj])
}

export const decryptObject = (encrypted_client, ) => {
  return null;
}
